...
const Typingtest = () => {
  ...
  const endGame = () => {
    ...
    const endTime = Date.now();
    const timeElapsedInMinutes = Math.max(1 / 60, (endTime - startTime) / 60000);
  ...
  return (
    <div className="typing-test-container">
      ...
      {gameOver && (
        <div className="results">
          <h2>Results</h2>
          <p>Words Per Minute: <span className="highlight">{wpm}</span></p>
          <p>Accuracy: <span className="highlight">{accuracy}%</span></p>
          <button onClick={initGame} className="restart-button">Try Again</button>
        </div>
      )}
    </div>
  );
